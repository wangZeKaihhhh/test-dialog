...
import { PhotoPreviewStore, PhotoPreviewContextType } from './context/PhotoPreviewProvider';
...
const PhotoPreviewContainer = (props: PhotoPreviewContainerProps) => {
  ...
  useLayoutEffect(() => {
    ...
    if (flipStatus === 'first' && previewVisibleStatus.current === 'show') {
      ...
      if (previewVisibleStatus.current === 'show') {
        ...
        lastRect.current = currentImage ? imgRef.current?.getBoundingClientRect() : initialDomRect;
  ...
  return previewVisibleStatus.current === 'show' || previewVisibleStatus.current === 'closing' ? (
    <>
      <div
        className="fixed left-0 top-0 bottom-0 right-0 bg-[#000] opacity-0 z-1 transition-all duration-300!"
        style={{
          opacity: flipStatus === 'play' && previewVisibleStatus.current !== 'closing' ? 0.9 : 0,
        }}
        onClick={() => {
      ...
      <img
        ...
        className={classNames([
          'fixed left-0 top-0 bottom-0 right-0 m-auto max-w-full max-h-full z-2',
          {
            'transition-all duration-400! ease-in-out':
              (previewVisibleStatus.current === 'show' && flipStatus === 'play') ||
              previewVisibleStatus.current === 'closing',
          },
        ])}
        src={currentImage}
        style={{
          transform:
            flipStatus === 'invert' || previewVisibleStatus.current === 'closing'
              ? `translate3d(${firstRect.current.left - lastRect.current.left}px,${
