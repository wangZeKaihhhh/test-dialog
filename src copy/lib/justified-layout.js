...
function addRow(layoutConfig, layoutData, row) {
  ...
  layoutData._layoutItems = [...layoutData._layoutItems, row.getItems()];
...
export default function (input, config) {
  ...
  containerPadding.right = !isNaN(parseFloat(layoutConfig.containerPadding.right))
    ? layoutConfig.containerPadding.right
    : layoutConfig.containerPadding;
  containerPadding.bottom = !isNaN(parseFloat(layoutConfig.containerPadding.bottom))
  ...
  return computeLayout(
    ...
    input.map(function (item) {
      if (item.width && item.height) {
        return { aspectRatio: item.width / item.height, url: item.url };
      } else {
        return { aspectRatio: item, url: item.url };
      }
    }),
  );
}
